...
function MindMapBody() {
    ...
    const handleGenerate = useCallback(async (customIds?: string[], customSources?: SourceMaterial[]) => {
        ...
        try {
            ...
            if (data.thread_id) {
                ...
                current.set("thread", data.thread_id);
    ...
    useEffect(() => {
        if (!course_id || !userEmail || hasAutoGenerated.current) return;
        setLoading(true);
        ...
        fetch(`${API_URL}/api/mindmap/sources/${course_id}?user_email=${userEmail}`)
            ...
            .then(data => {
                ...
                // Case A: Loading from thread ID (History)
                if (threadId && !graphData && !hasAutoGenerated.current) {
                    hasAutoGenerated.current = true;
                    handleGenerate(undefined, data);
                }
    ...
    }, [course_id, userEmail, searchParams, handleGenerate, graphData]); // handleGenerate is stable due to useCallback
    ...
    return (
        <div className="h-screen bg-[#050505] flex flex-row overflow-hidden font-sans">
            <Sidebar
                threads={threads}
                activeThreadId={searchParams.get("thread") || undefined}
